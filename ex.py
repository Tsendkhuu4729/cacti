...
def minimal_stateful_test(server):
    ...
    print('  |  '.join(['%s: %s' % (k.split('-')[-1], tx_ptks_dict[k]) for k in sorted(tx_ptks_dict.>

    print('RX by ports:')
    rx_ptks_dict = result.get_last_value('trex-global.data', 'ipackets-*')
    print('  |  '.join(['%s: %s' % (k.split('-')[-1], rx_ptks_dict[k]) for k in sorted(rx_ptks_dict.>
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Example for TRex Stateful, assuming server daemon >
