...
function form_actions() {
	...
	if (isset_request_var('selected_items')) {
		...
		if ($selected_items != false) {
			...
			} elseif (get_request_var('drp_action') == '9') { // SSH action
                
				foreach ($selected_items as $host_id) {
					$host_ip = db_fetch_cell_prepared('SELECT hostname FROM host WHERE id = ?', array($host_id));
					
					// Check if an SSH session already exists for this IP
					$session_exists = check_ssh_session($host_ip);
			
					if (!$session_exists) {
						$url = "http://192.168.10.107/cacti/terminal.html?host_ip=" . urlencode($host_ip);
			
						echo "<html><body>";
						echo "<script type='text/javascript'>";
						echo "window.open('" . $url . "', '_blank');"; // opens a new tab
						echo "</script>";
						echo "</body></html>";
							
						exit; // This will stop the script after opening the first SSH session
					} else {
						echo "<p>SSH session already active for IP: $host_ip</p>";
						raise_message('SSH_access_error', __('SSH session already active for IP \'%s\'', $host_ip), MESSAGE_LEVEL_WARN);
					}
	...
	foreach ($_POST as $var => $val) {
		...
		//	$host_ip .= '<li>' . html_escape(db_fetch_cell_prepared('SELECT hostname FROM host WHERE id = ?', array($matches[1]))) . '</li>';
	...
	if (isset($host_array) && cacti_sizeof($host_array)) {
		...
		}elseif (get_nfilter_request_var('drp_action') == '9') { // SSH access
			print "	<tr>
			 	<td colspan='2' class='textArea'>
			 		<p>" . __('Click \'Continue\' to SSH access.') . "</p>
			 		<div class='itemlist'><ul>$host_list </ul></div>
			 	</td>
			 	</tr>";

			 $save_html = "<input type='button' class='ui-button ui-corner-all ui-widget' value='" . __esc('Cancel') . "' onClick='cactiReturnTo()'>&nbsp;<input type='submit' class='ui-button ui-corner-all ui-widget' value='" . __esc('Continue') . "' title='" . __esc('SSH Access Device(s)') ."'>";
...
function check_ssh_session($host_ip) {
	// Uses the same db_fetch_cell_prepared function for database operations
	$count = db_fetch_cell_prepared("SELECT COUNT(*) FROM SSH_sessions WHERE ssh_host_ip = ?", array($host_ip));
	return $count > 0; // returns true if a session exists
}
